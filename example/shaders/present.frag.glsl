...
layout(binding = 0, set = 1) uniform sampler2D texLight;
layout(binding = 1, set = 1) uniform sampler2D texPrevLight;
...
vec3 sampleTextureCatmullRom(vec2 uv) {
    ...
    vec3 result = vec3(0);
    result += textureLod(texPrevLight, vec2(texPos0.x, texPos0.y), 0).rgb * w0.x * w0.y;
    result += textureLod(texPrevLight, vec2(texPos12.x, texPos0.y), 0).rgb * w12.x * w0.y;
    result += textureLod(texPrevLight, vec2(texPos3.x, texPos0.y), 0).rgb * w3.x * w0.y;

    result += textureLod(texPrevLight, vec2(texPos0.x, texPos12.y), 0).rgb * w0.x * w12.y;
    result += textureLod(texPrevLight, vec2(texPos12.x, texPos12.y), 0).rgb * w12.x * w12.y;
    result += textureLod(texPrevLight, vec2(texPos3.x, texPos12.y), 0).rgb * w3.x * w12.y;

    result += textureLod(texPrevLight, vec2(texPos0.x, texPos3.y), 0).rgb * w0.x * w3.y;
    result += textureLod(texPrevLight, vec2(texPos12.x, texPos3.y), 0).rgb * w12.x * w3.y;
    result += textureLod(texPrevLight, vec2(texPos3.x, texPos3.y), 0).rgb * w3.x * w3.y;
...
vec3 accumulateSampleAndWeights(ivec2 pos, out vec3 neighborhoodMin, out vec3 neighborhoodMax, out vec3 m1, out vec3 m2) {
    ...
    for (int x = -1; x <= 1; ++x ) {
        for (int y = -1; y <= 1; ++y ) {
            ...
            vec3 currentSample = texelFetch(texLight, pixelPosition, 0).rgb;
