...
layout(binding = 4, set = 1, rgba8) writeonly uniform image2D outColor;
...
void main() {
    ...
    vec2 texCoord = vec2(
        (floor(pixelPosition.x) + 0.5) / float(scene.resolution.x),
        (floor(pixelPosition.y) + 0.5) / float(scene.resolution.y));
    ...
    if (any(lessThan(reprojectedUV, vec2(0.0))) || any(greaterThan(reprojectedUV, vec2(1.0)))) {
        imageStore(outColor, pixelPosition, vec4(currentSample, 1.0));
        return;
    }
    ...
    imageStore(outColor, pixelPosition, vec4(result, 1.0));
}
